...
import vn.udn.dut.algoarena.port.mapper.HallMapper;
...
public class HallServiceImpl implements IHallService {
	...
	private Wrapper<Hall> buildQueryWrapper(HallBo bo) {
		...
		wrapper.eq(bo.getAlgoArenaId() != null, "h.algoarena_id", bo.getAlgoArenaId())
	...
	public Map<Long, List<HallVo>> getHallMap() {
		...
		Map<Long, List<HallVo>> hallMap = hallList.stream().collect(Collectors.groupingBy(HallVo::getAlgoArenaId));
		return hallMap;
	}
}
