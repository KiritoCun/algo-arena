...
import vn.udn.dut.algoarena.port.domain.Showtime;
...
import vn.udn.dut.algoarena.port.mapper.ShowtimeMapper;
import vn.udn.dut.algoarena.port.service.IAlgoArenaService;
import vn.udn.dut.algoarena.port.service.IHallSeatService;
...
public class ShowtimeServiceImpl implements IShowtimeService {
	...
	private Wrapper<Showtime> buildQueryWrapper(ShowtimeBo bo) {
		...
		wrapper.eq(bo.getAlgoArenaId() != null, "s.algoarena_id", bo.getAlgoArenaId())
	...
	public List<ShowtimeInfoVo> fetchShowtimeInfoList(Long movieId) {
		...
		Map<Long, List<ShowtimeVo>> showtimeMap = showtimeList.stream()
				.collect(Collectors.groupingBy(ShowtimeVo::getAlgoArenaId));
		...
		for (var entry : showtimeMap.entrySet()) {
			...
			Long algoArenaId = entry.getKey();
			AlgoArenaVo algoarena = algoarenaService.queryById(algoArenaId);
			showtimeInfoVo.setId(algoArenaId);
			showtimeInfoVo.setAlgoArena(algoarena);
